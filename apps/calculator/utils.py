...
def calc(img: Image, dict_of_vars: dict):
    ...
    prompt = (
        ...
        f"2. Set of Equations like x^2 + 2x + 1 = 0, 3y + 4x = 0, 5x^2 + 6y + 7 = 12, etc.: In this case, solve for all given variables, and the format should be a LIST OF DICTS, with each dict as {{'expr': 'variable', 'result': calculated_value, 'assign': True}}. Include as many dicts as there are variables. "
        ...
        f"IMPORTANT: For equations with multiple variables, make sure to include ALL variables in the output, not just one."
    )
